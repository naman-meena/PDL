...
#from pandapower.pypower.ext2int import ext2int

# ==============================================================================
# Power System Data (Using Pandapower)
# ==============================================================================
...
def get_pandapower_data(net, case_name):
    ...
    # --- Run a power flow to get the internal pypower case (ppc)
    # This is needed to build the Y-bus matrix
    # We use a dummy run, as we only need the ppc structure
    try:
    ...
    # Ensure ppc is built
    # Ensure ppc is built
    # We check if the _ppc attribute exists. If not, runpp() may have failed
    # and we need to run diagnostic() to build it.
    ...
    # --- Get Y-bus matrix ---
    # We need the internal bus numbering
    baseMVA = ppc['baseMVA']
    ...
    # --- Get generator buses ---
    # These are the bus indices where generators are connected
    gen_buses = net.gen.bus.values.tolist()

    # --- Get base load demands (as n_buses vectors) ---
    ...
    if net.poly_cost.shape[0] == n_generators:
        ...
        # --- Convert polynomial costs from MW to p.u. ---
        # Cost = c2 * P_mw^2 + c1 * P_mw + c0
        # P_mw = P_pu * baseMVA
        # Cost = c2 * (P_pu * baseMVA)^2 + c1 * (P_pu * baseMVA) + c0
        # Cost = (c2 * baseMVA^2) * P_pu^2 + (c1 * baseMVA) * P_pu + c0
        c2_pu = c2 * (baseMVA**2)
        c1_pu = c1 * baseMVA
        c0_pu = c0
    ...
    # --- Get voltage limits (p.u.) ---
    # The NN model uses single min/max values
    V_min = net.bus.min_vm_pu.min()
    V_max = net.bus.max_vm_pu.max()

    # --- Get slack bus ---
    # Assumes external grid is the slack
...
# ==============================================================================
# Neural Network Architectures (Unchanged)
# ==============================================================================
...
class ACOPFPrimalNet(nn.Module):
    ...
    def __init__(self, n_buses, n_generators, hidden_dim=256):
        ...
        # Activation for bounded outputs
    ...
    def forward(self, P_demand, Q_demand, gen_limits, V_min, V_max, slack_bus=0):
        ...
        # Generator active power (with bounds)
        ...
        # Generator reactive power (with bounds)
        ...
        # Voltage magnitudes (with bounds)
        v_raw = self.sigmoid(self.v_net(h))
        V = V_min + v_raw * (V_max - V_min)

        # Voltage angles (slack bus = 0)
...
class ACOPFDualNet(nn.Module):
    ...
    def __init__(self, n_buses, hidden_dim=256):
        ...
        input_dim = 2 * n_buses
        output_dim = 2 * n_buses  # Multipliers for P and Q balance
...
# ==============================================================================
# PDL for AC-OPF (Unchanged logic, now uses p.u. values)
# ==============================================================================
...
class PDL_ACOPF:
    ...
    def __init__(self, system_data, rho_init=1.0, rho_max=1000.0,
        ...
        # Y_bus from pandapower is already complex
    ...
    def compute_power_balance(self, Pg, Qg, V, theta, P_demand, Q_demand):
        ...
        # Create net power injection vectors
        ...
        # Add generation (Pg, Qg are p.u.)
        # We need to map generator outputs to their respective buses
        ...
        # Note: Y_bus is (n_buses, n_buses)
        # V_complex is (batch_size, n_buses)
        # We need to use matmul for batch operations
    ...
    def primal_loss(self, Pg, Qg, V, theta, P_demand, Q_demand, multipliers):
        """Primal loss: cost + Lagrangian + penalty"""
        # Generation cost
    ...
    def train_epoch(self, P_demand_batch, Q_demand_batch, inner_iters=250):
        ...
        # Compute violations and update rho
        with torch.no_grad():
            Pg, Qg, V, theta = self.primal_net(
                P_demand_batch, Q_demand_batch,
                self.gen_limits, self.V_min, self.V_max, self.slack_bus
...
# ==============================================================================
# Plotting Functions (Modified to show MW/MVAr)
# ==============================================================================
...
# ==============================================================================
# Main Execution (Modified to use pandapower)
# ==============================================================================
